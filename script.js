...
const NUTRITION_DB = {
  ...
  pork:      [242, 27, 0,   14,   0],    salmon:  [208, 20, 0,   13,  0],
  tuna:      [130, 29, 0,    1,   0],    shrimp:  [99,  24, 0.2, 0.3, 0],
  fish:      [136, 24, 0,    5,   0],    egg:     [155, 13, 1,   11,  0],
  eggs:      [155, 13, 1,   11,   0],    tofu:    [76,   8, 2,    4,  0.3],
  turkey:    [189, 29, 0,    7,   0],    lamb:    [294, 25, 0,   21,  0],
  broccoli:  [34,  2.8, 7,  0.4, 2.6],  spinach: [23,  2.9, 3.6, 0.4, 2.2],
  zucchini:  [17,  1.2, 3.1, 0.3, 1],   carrot:  [41,  0.9, 10,  0.2, 2.8],
  potato:    [77,  2,  17,  0.1, 2.2],  tomato:  [18,  0.9, 3.9, 0.2, 1.2],
  mushroom:  [22,  3.1, 3.3, 0.3, 1],   eggplant:[25,  1,   6,   0.2, 3],
  pepper:    [31,  1,   6,   0.3, 2.1], asparagus:[20, 2.2, 3.9, 0.1, 2.1],
  lentil:    [116, 9,  20,  0.4, 7.9],  lentils: [116, 9,  20,  0.4, 7.9],
  chickpea:  [164, 8.9, 27, 2.6, 7.6],  chickpeas:[164,8.9, 27, 2.6, 7.6],
  beans:     [127, 8.7, 22, 0.5, 6.4],  rice:    [130, 2.7, 28, 0.3, 0.4],
  pasta:     [158, 5.8, 31, 0.9, 1.8],  quinoa:  [120, 4.4, 21, 1.9, 2.8],
  cheese:    [402, 25,  1.3, 33,  0],   yogurt:  [59,  10,  3.6, 0.4, 0],
  avocado:   [160, 2,   9,  15,  6.7],  lemon:   [29,  1.1, 9,   0.3, 2.8],
};

const METHOD_CALORIES = {
  'Fried': 200, 'Air Fried': 60, 'Sautéed': 80, 'Baked': 30,
  'Roasted': 40, 'Grilled': 20, 'Steamed': 0, 'Raw / No-Cook': 0,
  'Slow Cooked': 20, 'Instant Pot': 10,
};
...
async function generateRecipe() {
  ...
  if (recipesLeft() <= 0) { openPaywall(); return; }
...
function renderRecipe(r) {
  ...
  document.getElementById('instructionsList').innerHTML = r.steps.map(s => `<li>${s}</li>`).join('');
...
function buildSteps(ing, meth) {
  ...
  const cook = {
    'Baked':         [`Preheat oven to 400°F (200°C). Line a baking dish with parchment.`, `Toss ${i} with oil, garlic, onion, and seasonings.`, `Arrange in a single layer. Add extras on top.`, `Bake 25–35 min until golden at the edges.`, `Finish with fresh herbs or a squeeze of lemon.`],
    'Grilled':       [`Preheat grill to medium-high. Brush with oil.`, `Marinate ${i} with oil, garlic, and seasonings for 10 min.`, `Grill 4–6 min per side until nicely charred.`, `Rest 5 min before slicing. Finish with herbs or sauce.`],
    'Roasted':       [`Preheat oven to 425°F (220°C).`, `Toss ${i} with oil, garlic, onion, and seasonings on a sheet pan.`, `Spread in a single layer — don't crowd the pan.`, `Roast 25–40 min, tossing halfway, until caramelized.`, `Drizzle with balsamic and a pinch of finishing salt.`],
    'Sautéed':       [`Heat oil in a large skillet over medium-high until shimmering.`, `Add onion, cook 3 min. Add garlic, cook 1 min more.`, `Add ${i} and cook, stirring, for 8–12 min.`, `Deglaze with a splash of wine or broth.`, `Season, finish with herbs, and serve immediately.`],
    'Slow Cooked':   [`Add ${i}, onion, garlic, and broth to slow cooker.`, `Stir in all seasonings and herbs.`, `Cook on LOW 6–8 hrs or HIGH 3–4 hrs.`, `Taste and adjust seasoning. Remove bay leaf.`, `Serve over rice, bread, or as-is.`],
    'Air Fried':     [`Preheat air fryer to 380°F (193°C).`, `Toss ${i} with oil and seasonings.`, `Arrange in a single layer — cook in batches if needed.`, `Air fry 12–18 min, shaking halfway.`, `Finish with herbs and a squeeze of lemon.`],
    'Raw / No-Cook': [`Slice or tear ${i} to desired size.`, `Whisk together oil, lemon juice, garlic, and a pinch of salt.`, `Toss with dressing and aromatics.`, `Let sit 5–10 min for flavors to meld.`, `Taste, adjust acid and salt, and serve fresh.`],
  };
...
function loadSaved(i) {
  ...
  document.querySelectorAll('.pill').forEach(p => {
    p.classList.toggle('active', !!r[p.dataset.group] && r[p.dataset.group] === p.dataset.value);
  });
  toggleSaved();
}
...
function updateAnotherBtn() {
  ...
  if (!btn) return;
  btn.innerHTML = lastIngredient ? `🔄 New ${capitalize(lastIngredient)} Recipe` : '🔄 New Recipe';
}
